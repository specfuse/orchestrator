...
so you can drive them via scripts/qa-feature-dispatcher.py.
...
Usage:
    python3 scripts/runner.py                              # newest INIT-*.md
    python3 scripts/runner.py --feature features/INIT-2026-0001.md
    python3 scripts/runner.py --dry-run                    # no writes, no dispatch
    python3 scripts/runner.py --approve                    # approve gate then continue
"""
...
try:
    ...
    sys.stderr.write("error: pyyaml required. install: pip install -r scripts/requirements.txt\n")
    sys.exit(3)
...
def _recommended_actions(impl_entries: list[dict], feature_status: dict[str, str],
    ...
    for fid in sorted(by_status.get("blocked", [])):
        actions.append(
            ...
            f"resolve the blocker, then re-run `python3 scripts/runner.py`."
        )
    ...
    for fid in review_fids:
        actions.append(
            ...
            f"plan-next body), then re-run `python3 scripts/runner.py --approve` to pass the gate."
        )
    # Unarmed draft WUs.
    unarmed_fids = sorted(by_status.get("unarmed_now", []) + by_status.get("unarmed", []))
    for fid in unarmed_fids:
        actions.append(
            f"ARM {fid}: re-run `python3 scripts/runner.py --approve` to arm the draft WUs."
        )
    ...
    for fid in main_fids:
        actions.append(
            f"BOOTSTRAP {fid}: re-run `python3 scripts/runner.py` — the loop's first pass will "
    ...
    for fid in not_adopted:
        actions.append(
            ...
            f"re-run `python3 scripts/runner.py` to retry, or invoke "
    ...
    if pending and not (review_fids or unarmed_fids):
        actions.append(
            ...
            f"`python3 scripts/runner.py` to advance their gates."
        )
    ...
    if unblocked_qa:
        ...
        actions.append(
            ...
            f"`python3 scripts/qa-feature-dispatcher.py` (runner does not drive QA)."
        )
    if blocked_qa:
    ...
    if all_impl_done and not qa_ready:
        actions.append(
            "INITIATIVE COMPLETE — `python3 scripts/poller.py` will emit the "
