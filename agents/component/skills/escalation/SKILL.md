...
3. Construct each accompanying event (`task_blocked`, `human_escalation`, and when applicable `spec_issue_raised`) and pipe every line through `specfuse validate-event`. Append to the feature event log only once the validator exits `0` for every line. Exit `1` sends the event back to construction; exit `2` (setup error) is itself a `spec_level_blocker` — do not loop.
...
- `1.1` — WU 1.7: the escalation event flow (§"Event emission: what goes where" step 3) now requires every event line to exit `0` from `specfuse validate-event` before the append. Finding 1 of the Phase 1 walkthrough retrospective.
- `1.0` — Phase 1 initial.
