...
- **Event log entries** appended to `/events/FEAT-YYYY-NNNN.jsonl` in the orchestration repo: `task_started` (on pickup, after the state transition), `task_completed` (emitted only after verification passes), `task_blocked` (on any `in_progress → blocked_*` transition), `override_applied` / `override_expired` for override lifecycle, `human_escalation` for escalations, `spec_issue_raised` when filing spec issues. Every event must be piped through [`scripts/validate-event.py`](../../scripts/validate-event.py) and exit `0` before the append; see [`verify-before-report.md`](../../shared/rules/verify-before-report.md) §3 for exit-code semantics.
