...
- Emit event log entries for every feature-level state transition the role owns, round-tripped through [`event.schema.json`](../../shared/schemas/event.schema.json) via `specfuse-validate-event`.
...
- **Event log entries** appended to `/events/FEAT-YYYY-NNNN.jsonl` for feature state transitions the role owns: `feature_created` (on feature intake), `feature_state_changed` (on `drafting → validating` and `validating → planning`), and `human_escalation` (on escalations). Every event is piped through `specfuse-validate-event` and must exit `0` before the append; the `source_version` field is produced by `python3 -m specfuse.orchestrator._version` at emission time, never eye-cached from `version.md`. See [`verify-before-report.md`](../../shared/rules/verify-before-report.md) §3 for the full construction discipline (including the canonical `--file /tmp/event.json` validate-event.py invocation, the JSONL single-line requirement, the canonical safe append pattern, and the timestamp discipline — all absorbed in WU 3.11).
