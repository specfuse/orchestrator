...
- Every event is piped through `specfuse validate-event` and must exit `0` before the append; the `source_version` field is produced by `python3 -m specfuse.orchestrator._version` at emission time, never eye-cached from `version.md`. See [`verify-before-report.md`](../../shared/rules/verify-before-report.md) §3 for the full construction discipline (including the canonical `--file /tmp/event.json` validate-event.py invocation, the JSONL single-line requirement, the canonical safe append pattern, and the timestamp discipline — all absorbed in WU 3.11).
...
5. **Emitting duplicate `qa_execution_*` events for the same `(task_correlation_id, manifest_hash)` pair (falling back to `(task_correlation_id, commit_sha)` when the run carries no `stack_manifest`).** The idempotence discipline is what lets execution be safely replayed across poller cycles; duplicate events corrupt the audit trail and can cascade into duplicate regression artifacts.
