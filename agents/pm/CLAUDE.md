...
- **Event log entries** appended to `/events/FEAT-YYYY-NNNN.jsonl` in the orchestration repo: `task_graph_drafted` on task-graph completion (envelope-only at v1.6.3), `plan_ready` on `planning → plan_review` (envelope-only), `task_created` on each issue open (envelope-only; payload shape documented in [`skills/issue-drafting/SKILL.md`](skills/issue-drafting/SKILL.md) §"Event payloads"), `task_ready` on every `pending → ready` flip — no-dep issue creation and dependency recomputation (envelope-only; payload shape documented in [`skills/issue-drafting/SKILL.md`](skills/issue-drafting/SKILL.md) §"Event payloads" and [`skills/dependency-recomputation/SKILL.md`](skills/dependency-recomputation/SKILL.md) §"Event payload — `task_ready`"), `feature_state_changed` on each feature-level transition owned by this role — payload schema: [`shared/schemas/events/feature_state_changed.schema.json`](../../shared/schemas/events/feature_state_changed.schema.json) (see enumeration below), `template_coverage_checked` emitted by the template-coverage-check skill — payload schema: [`shared/schemas/events/template_coverage_checked.schema.json`](../../shared/schemas/events/template_coverage_checked.schema.json), and `human_escalation` on escalations — payload schema: [`shared/schemas/events/human_escalation.schema.json`](../../shared/schemas/events/human_escalation.schema.json). Events without an inline per-type schema reference are envelope-only at v1.6.3; adding per-type schemas for envelope-only task lifecycle event types is explicitly deferred to Phase 4+ per the Phase 3 retrospective (F3.15). Every event is piped through `specfuse validate-event` and must exit `0` before the append; the `source_version` field is produced by `python3 -m specfuse.orchestrator._version` at emission time, never eye-cached from `version.md`. See [`verify-before-report.md`](../../shared/rules/verify-before-report.md) §3 for the full construction discipline (including the canonical `--file /tmp/event.json` validate-event.py invocation, the JSONL single-line requirement, the canonical safe append pattern, and the timestamp discipline — all absorbed in WU 3.11).
