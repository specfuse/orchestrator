...
- [`qa_regression_filed.schema.json`](qa_regression_filed.schema.json) — `payload.implementation_task_correlation_id` (required, task-level — the ORIGINAL impl task regressed against, resolved from the feature task graph per Q4), `payload.test_id` (required, kebab-case), `payload.failing_qa_execution_event_ts` (required, ISO-8601 backlink), `payload.failing_commit_sha` (required, 40-char), `payload.regression_inbox_file` (required, path under `inbox/qa-regression/`). Authored 2026-04-23 in WU 3.4. Idempotence key `(implementation_task_correlation_id, test_id)` — a second filing for the same pair is forbidden (repeat failures escalate `spinning_detected` instead). No `spawned_implementation_task_correlation_id` field: QA does not mint task-level correlation IDs; the PM inbox consumer or the human does, later.
- [`qa_regression_resolved.schema.json`](qa_regression_resolved.schema.json) — `payload.implementation_task_correlation_id` + `payload.test_id` (required; match the paired `qa_regression_filed`), `payload.filed_event_ts` (required, ISO-8601 backlink to the paired filed event), `payload.resolving_qa_execution_event_ts` (required, ISO-8601), `payload.resolving_commit_sha` (required, 40-char; must differ from the paired filed event's `failing_commit_sha`). Authored 2026-04-23 in WU 3.4.
- [`escalation_resolved.schema.json`](escalation_resolved.schema.json) — substrate event; `payload.resolution_kind` (required enum discriminator: `qa_regression_resolved`, `human_resolved`), `payload.resolved_escalation_event_ts` (required, ISO-8601 backlink to the `human_escalation` being retired), `payload.resolved_escalation_inbox_file` (optional path under `inbox/human-escalation/`), `payload.summary` (required prose). Authored 2026-04-23 in WU 3.4 as the absorption of Phase 2 carry-item F2.10. The envelope `correlation_id` of an `escalation_resolved` MUST equal the `correlation_id` of the `human_escalation` it resolves (feature-level or task-level accordingly). First application: retires `inbox/human-escalation/FEAT-2026-0005-plan-review-cycle.md` via a `human_resolved` instance appended to `events/FEAT-2026-0005.jsonl` in WU 3.4's commit.
- [`feature_created.schema.json`](feature_created.schema.json) — `payload.feature_title` (required, string), `payload.involved_repos` (required, array of `owner/repo` strings), `payload.autonomy_default` (required, enum: `auto`, `review`, `supervised`), `payload.correlation_id` (required, `FEAT-YYYY-NNNN` pattern — duplicated from envelope for payload self-containment). Authored 2026-04-25 in WU 4.2; first specs-agent-emitted per-type schema. The `feature_created` event type has existed in the Phase 1 baseline enum; this per-type schema is additive.
